...
│   └── config.py
├── data/
│   └── incoming/      # Папка для файлов, которые нужно обработать, используется для 3 задания, для 1 и 2 файлы формируются автоматически
│   └── logs/
...
4) Для выполнения заданий 1 и 2 можно запустить скрипт `run_pipeline_task1&2.py`.</br>
