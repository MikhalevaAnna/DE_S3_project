# Проект: Асинхронный пайплайн обработки данных с S3 хранилищем Selectel Cloud Storage
## Выполнить задания: 
### I:
...
1) Настраиваю `bucket policy` таким образом, чтобы любой может читать файлы из определенного бакета и только я могла писать в него (`account_root`), скриншот (Access_policy.png): </br>
...
9) Затем, для восстановленного файла demo_versioning.txt, необходимо нажать справа 3 вертикальные точки и выбрать Скачать.</br>
...
├── data/
│   └── logs/
│   │   └── pipeline_tasks_1&2_20260108_194148.log # Результат выполнения заданий 1 и 2 можно посмотреть в этом логе
```
Результат работы скрипта для 1 и 2 заданий, как пример, находится в папке *logs*. Кроме этого, файлы, скачанные из S3, </br>
...
3) Для выполнения задания 3 необходимо запустить скрипт `run_pipeline_task3.py` он обращается к скрипту `pipeline.py`, который находится в папке `src`.</br>
Результатом работы скрипта будут логи вида: </br>
...
│   └── logs/
│   │   └── pipeline_log_2026-01-08.json  
|   │   └── pipeline_task3_20260108_195619.log # Результат выполнения 3 задания можно посмотреть в этих логах
```
...
a) После запуска run_pipeline_task3.py`в директории **data/incoming** создастся файл mployees_example.csv со столбцом `salary` для того, чтобы получилось </br>
отфильтровать данные по зарплате. </br>
b) Параметры фильтра по зарплате необходимо указать в файле `config.py` директории `config`. </br>
...
c) В локальной директории в архиве лежат исходные файлы, и файлы, полученные после работы пайплана. </br>
...
|   |   └── archive/
|   |   |   └── 2026-01-08/
|   |   │   │   └── employees_example.csv # Исходный файл, который автоматически создается в папке incoming для демонстрации работы пайплайна
|   │   |   |   └── salary_filtered_employees_example_1767891380.csv # Полученный файл, в результате работы пайплайна
|   │   |   |   └── salary_filtered_employees_example_1767891434.csv # Полученный файл, в результате работы пайплайна
|   │   |   |   └── salary_filtered_test_data_1767891387.csv # Полученный файл, в результате работы пайплайна
|   │   |   |   └── test_data.csv # Исходный файл, который был скопирован в incoming обрабатывался в папке incoming
```
